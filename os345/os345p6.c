// os345p6.c - FAT File Management System	06/21/2020
// ***********************************************************************
...
void outFatEntry(int index)
{
    ...
    if (index < 2)
        printf("RSRV"); // A reserved cluster
    else if (fatvalue == FAT_EOC)
        printf("EOC"); // The EOC marker
    else if (fatvalue == FAT_BAD)
        printf("BAD"); // The BAD cluster marker
    else
        printf("%d", fatvalue);
    return;
}
...
int P6_mount(int argc, char *argv[]) // mount RAM disk
{
    ...
    if (argc < 2)
        strcat(temp, "c:/lcc/projects/disk4");
    else
        strcat(temp, argv[1]);
    printf("\nMount Disk \"%s\"", temp);
...
int P6_fileSlots(int argc, char *argv[]) // list open file slots
{
    ...
    for (fd = 0; fd < NFILES; fd++)
    {
        ...
        for (i = 0; i < 12; i++)
            printf("%c", fcb->name[i]);
        printf("  %02x%6u%6u%6u%6u%6u%6u%6u%6u", fcb->attributes, fcb->fileSize, fcb->startCluster, fcb->currentCluster,
               fcb->directoryCluster, fcb->pid, fcb->mode, fcb->flags, fcb->fileIndex);
    }
    return 0;
} // end P6_fileSlots
...
void printDirectoryEntry(DirEntry *dirent)
{
    ...
    sprintf(&p_string[31], "%02u/%02u/%04u %5u %5u", date.month + 1, date.day, date.year + 1980, dirent->startCluster,
            dirent->fileSize);
...
void printFatEntries(unsigned char *FAT, int start, int end)
{
    ...
    for (i = start; i < nn; i++)
    {
        ...
        if (i < 2)
            ...
            sprintf(tbuf, "  EOC"); // The EOC marker
        else if (fatvalue == FAT_BAD)
            sprintf(tbuf, "  BAD"); // The BAD cluster marker
        else
            sprintf(tbuf, "%5u", fatvalue);
        strcat(fbuf, tbuf);
        counter++;
    }
    if (counter % 10)
...
int P6_chkdsk(int argc, char *argv[]) // check RAM disk
{
    ...
    for (i = 0; i < CLUSTERS_PER_DISK; i++)
    {
        ...
        {
            printf("\n  FAT1[%d] != FAT2[%d] (%d/%d)", i, i, getFatEntry(i, ckFAT1), getFatEntry(i, ckFAT2));
        }
    }
    return 0;
} // end P6_chkdsk
...
void checkDirectory(char *dirName, unsigned char fat[], int dir)
{
    ...
    while (1)
    {
        if (fmsGetNextDirEntry(&index, "*.*", &dirEntry, dir))
            break;
        // process file name
        getFileName(fileName, &dirEntry);
...
int fmsTests(int test, bool debug)
{
    ...
    int i, error;
    int tFID[numFiles];
    char *text[numWords] = {"Now ", "is ",   "the ", "time ", "for ", "all ", "good ",  "men ",
    ...
    case 3: {
        ...
        rBuf[0] = 0;
        while ((error = fmsReadFile(tFID[test2], buf, 1)) > 0)
            strncat(rBuf, buf, 1);
    ...
    case 4: {
        ...
        int t3FID;
        int fileSize = 10 * 512;
        int index[numWords] = {510, 20, 5120, 1024, 0, 4095, 4000, 5000, 100, 5090, 3000, 2000, 1535, 3100, 1900, 4500};
...
int isValidFileName(char *fileName)
{
    ...
    {
        if (strchr(s + 1, '.'))
            return 0; // more than 1 '.'
        if (strlen(s + 1) > 3)
            return 0; // too long of extension
        if ((strlen(fileName) - strlen(s)) <= 8)
            return 1;
        else
            return -1;
    }
...
int fmsMask(char *mask, char *name, char *ext)
{
    ...
    for (i = 0, j = 0; j < 8; i++, j++)
    {
        ...
        if ((mask[i] == '?') && (name[j] != ' '))
            continue;
        if (!mask[i] && (name[j] == ' ') && (ext[0] == ' '))
            return 1;
        if ((mask[i] != toupper(name[j])) && (mask[i] != tolower(name[j])))
            return 0;
    }
    while (mask[i] == '.')
        i++;
    ...
    for (j = 0; j < 3; i++, j++)
    {
        ...
        if ((mask[i] == '?') && (ext[i] != ' '))
            continue;
        if ((mask[i] != toupper(ext[j])) && (mask[i] != tolower(ext[j])))
            return 0;
    }
    return 1;
} // end fmsMask
...
//	Return 0 for success, otherwise, return the error number.
{
    ...
    if ((error = fmsGetDirEntry(dirName, &dirEntry)))
        return error;
...
void sizeDisk(DiskSize *dskSize, char *mask, int dir)
{
    ...
    while (1)
    {
        if (fmsGetNextDirEntry(&index, mask, &dirEntry, dir))
            break;
        if (dirEntry.name[0] == '.')
            continue;
