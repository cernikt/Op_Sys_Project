...
int P2_main(int argc, char *argv[])
{
    ...
    createTask("10 Seconds", timeTask, HIGH_PRIORITY, 0, NULL);
	createTask("10 Seconds", timeTask, HIGH_PRIORITY, 0, NULL);
	createTask("10 Seconds", timeTask, HIGH_PRIORITY, 0, NULL);
	createTask("10 Seconds", timeTask, HIGH_PRIORITY, 0, NULL);
	createTask("10 Seconds", timeTask, HIGH_PRIORITY, 0, NULL);
...
int P2_listTasks(int argc, char *argv[])
{
    ...
    //             printQueue(&tcb[i].event->blocked_q);
...
int timeTask(int argc, char *argv[]) {
    while (1) {
        char timeString[80]; // string to hold time
        // ?? print the task id and the current time using the above function
        printf("\ntid: [%d], time: %s\n", curTask, myTime(timeString));
