               ...
               char *argv[])              // task argument pointers
{
    ...
    for (tid = 0; tid < MAX_TASKS; tid++)
    {
        if (tcb[tid].name == 0)
        {
            ...
            //tcb[tid].argv = argv; // argument pointers String dupe or malloc. Duplicate argv into tcb.argv variable
...
int sysKillTask(int taskId)
{
    ...
    tcb[taskId].name = 0; // release tcb slot Free memory that was malloced
    return 0;
} // end sysKillTask
