...
static int scheduler()
{
    ...
    if (nextTask != -1 && tcb[nextTask].name && tcb[nextTask].signal & mySIGSTOP) {
        //printf("inside stop sgnl if\n");
        enQueue(&pq, nextTask);
        return -1;
    }
...
static int initOS()
{
    ...
    //printf("malloced ready q and got time\n");
...
int enQueue(PriorityQueue* priority_q, TID tid) {
    // insert tid on q with priority tcb[tid].priority
    // return 0 on success, -1 on failure

    // if queue is empty, insert at head
    //printf("\nenqueuing task %d\n", tid);

    PqTask* newTask = (PqTask*)malloc(sizeof(PqTask));
    if (priority_q->head == NULL) {
        priority_q->head = newTask;
        priority_q->head->tid = tid;
        priority_q->head->next = NULL;
        return 0;
    } else  {
        PqTask* cur = priority_q->head;

        while (cur->next && tcb[cur->next->tid].priority >= tcb[tid].priority) {
            cur = cur->next;
        }

        newTask->tid = tid;
        newTask->next = cur->next;
        cur->next = newTask;
        return 0;
    }
    return -1;
...
int deQueue(PriorityQueue* priority_q) {
    // remove and return tid with highest priority (should always be the head)
...
void printQueue(PriorityQueue* priority_q) {
    PqTask* cur = priority_q->head;
    printf("Queue Contents\n");
    while(cur) {
        printf("[%d] priority: %d\n", cur->tid, tcb[cur->tid].priority);
        cur = cur->next;
    }
    printf("\n");
}
...
int del_task(PriorityQueue* priority_q, TID tid) {
    // search queue and delete node with given tid
    // return 0 on success, -1 on failure
