...
int lc3Task(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        case LC3_ADD: // ADD instruction
            ...
            {
                if (LC3_DEBUG & 0x01)
                    printf(ADD_MSG, oldpc, DR, GET_SR1, GET_SR2);
                LC3_REGS[DR] = MASKTO16BITS(LC3_REGS[GET_SR1] + LC3_REGS[GET_SR2]);
            }
            else
            {
                if (LC3_DEBUG & 0x01)
                    printf(ADDI_MSG, oldpc, DR, GET_SR1, MASKTO16BITS(SEXT5(ir)));
        ...
        case LC3_AND: // AND instruction
            ...
            {
                if (LC3_DEBUG & 0x01)
                    printf(AND_MSG, oldpc, DR, GET_SR1, GET_SR2);
                LC3_REGS[DR] = MASKTO16BITS(LC3_REGS[GET_SR1] & LC3_REGS[GET_SR2]);
            }
            else
            {
                if (LC3_DEBUG & 0x01)
                    printf(ANDI_MSG, oldpc, DR, GET_SR1, MASKTO16BITS(SEXT5(ir)));
                LC3_REGS[DR] = MASKTO16BITS(LC3_REGS[GET_SR1] & SEXT5(ir));
            }
            SET_CC(LC3_REGS[DR]);
            break;
        ...
        case LC3_JMP: // JMP instruction
            ...
            {
                if (GET_BASER == 7)
                    printf(RET_MSG, oldpc);
                else
                    printf(JMP_MSG, oldpc, GET_BASER);
            }
        ...
        case LC3_JSR: // JSR instruction
            ...
            {
                if (LC3_DEBUG & 0x01)
                    printf(JSRR_MSG, oldpc, GET_BASER);
                LC3_PC = LC3_REGS[GET_BASER];
            }
            else
            {
                if (LC3_DEBUG & 0x01)
                    printf(JSR_MSG, oldpc, LC3_PC + SEXT11(ir));
        ...
        case LC3_LD: // LD instruction
            if (LC3_DEBUG & 0x01)
                printf(LD_MSG, oldpc, DR, SEXT9(ir));
        ...
        case LC3_LDI: // LDI instruction
            if (LC3_DEBUG & 0x01)
                printf(LDI_MSG, oldpc, DR, SEXT9(ir));
        ...
        case LC3_LDR: // LDR instruction
            if (LC3_DEBUG & 0x01)
                printf(LDR_MSG, oldpc, DR, GET_BASER, SEXT6(ir));
        ...
        case LC3_LEA: // LEA instruction
            if (LC3_DEBUG & 0x01)
                printf(LEA_MSG, oldpc, DR, LC3_PC + SEXT9(ir));
        ...
        case LC3_NOT: // NOT instruction
            if (LC3_DEBUG & 0x01)
                printf(NOT_MSG, oldpc, DR, GET_SR1);
        ...
        case LC3_TRAP: // TRAP instruction
        {
            ...
            case LC3_PUTS: {
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "PUTS");
                string_address = LC3_REGS[0];
                while ((tmp = getMemoryData(string_address++)) != 0)
                    putchar(tmp);
                LC3_REGS[7] = MASKTO16BITS(LC3_PC);
                break;
            }
            ...
            case LC3_getNextDirEntry: {
                ...
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsGetNextDirEntry");
            ...
            case LC3_closeFile: {
                ...
                // OUT:  R0 = 0-success, otherwise error
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsCloseFile");
            ...
            case LC3_defineFile: {
                ...
                if (!s[j])
                    j++;
                while ((fileName[i++] = s[j]))
                    j += 2;
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsCreateFile");
            ...
            case LC3_deleteFile: {
                ...
                while ((fileName[i++] = s[j]))
                    j += 2;
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsDeleteFile");
            ...
            case LC3_openFile: {
                ...
                while ((fileName[i++] = s[j]))
                    j += 2;
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsOpenFile");
            ...
            case LC3_readFile: {
                ...
                // OUT:  R0 = 0-success, otherwise error
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsReadFile");
            ...
            case LC3_seekFile: {
                ...
                // OUT:  R0 = 0-success, otherwise error
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsSeekFile");
            ...
            case LC3_writeFile: {
                ...
                // OUT:  R0 = 0-success, otherwise error
                if (LC3_DEBUG & 0x01)
                    printf(TRAP_MSG, oldpc, "fmsWriteFile");
...
int getCharacter()
{
    ...
    SEM_WAIT(charReady)
    if (inChar == 13)
        inChar = 10; // if (inChar == CR) inChar = LF
