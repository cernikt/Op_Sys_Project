...
int jurassicTask(int argc, char *argv[])
{
    ...
    moveCars = createSemaphore("moveCar", BINARY, 0);
    SWAP;
    ...
    for (i = 0; i < NUM_CARS; i++)
    {
        ...
        SWAP;
        fillSeat[i] = createSemaphore(buf, BINARY, 0);
        SWAP;
        sprintf(buf, "seatFilled%d", i);
        SWAP;
        seatFilled[i] = createSemaphore(buf, BINARY, 0);
        SWAP;
        sprintf(buf, "rideOver%d", i);
        SWAP;
        rideOver[i] = createSemaphore(buf, BINARY, 0);
        SWAP;
    }
    ...
    parkMutex = createSemaphore("parkMutex", BINARY, 1);
    SWAP;
        ...
        for (i = 0; i < NUM_CARS; i++)
        {
            ...
            if (makeMove(c[i]))
            {
                    ...
                    (myPark.numInCarLine))
                {
                    ...
                    ParkDebug(buf);
                    SEM_SIGNAL(fillSeat[c[i]]);
                    SWAP; // seat open

                    sprintf(buf, "SEM_WAIT(seatFilled[%d])", c[i]);
                    ParkDebug(buf);
                    SEM_WAIT(seatFilled[c[i]]);
                    SWAP; // passenger in seat
